...
{
  if (isIntersecting) {
    let modals = Array.from(document.getElementsByClassName('modal fade'))
    if (!modals.some(modal => modal.style.display === 'block')) window.location.assign('#projects');
  }
})
...
renderer.setAnimationLoop( () => {
  ...
  if (bagtip){
    ...
    bagtip.style.left = `${(pointer.x+1)*50 - 5}vw`;
  } 
